...
```bash
mvn test                  # 69건 (엔진 시나리오 + 전 스펙 기능 end-to-end)
...
원 스펙 `Continuum.md` §3의 요구 기능을 전량 구현했다. 각 기능은 독립 커밋 + 직접 실행(엔진+H2 DB end-to-end) 테스트로 검증된다.

| 스펙 | 기능 | 상태 |
|---|---|---|
| §3.2.1 | 상태머신 `CREATED/RUNNING/WAITING/COMPLETED/FAILED/CANCELED` + 명시적 전이 검증 | ✅ `ExecutionStatus`, `StateMachineTests` |
| §3.1.1 | DSL `workflow_id`/`version`/`transitions`/`failure_policy` + step 타입 검증 | ✅ `WorkflowDef`/`StepKind`, `DslValidationTests` |
| §3.1.2 | 워크플로우 버전 관리 (실행 중 정의 변경 격리, 이력 보존) | ✅ `WorkflowEntity.version`, `VersioningTests` |
| §3.1.1 | `CONDITIONAL_STEP` + transitions 분기 | ✅ `ConditionEvaluator`, `ConditionalStepTests` |
| §3.1.1 | `HUMAN_APPROVAL_STEP` + 승인/거부 API | ✅ `WAITING` 파킹, `HumanApprovalTests` |
| §3.4 | 내부 이벤트 버스 (완료/실패/타임아웃, 비동기 + 손실방지) | ✅ `EventBus`, `EventBusTests` |
| §3.6.1 | 감사 로그 (누가/언제/왜 상태 전이) | ✅ `AuditLog`, `AuditLogTests` |
| §3.7 | 관리자 기능 (강제중단/스텝 재실행/강제 상태변경/dead 탐지/통계) | ✅ `AdminController`, `AdminTests` |
| §3.6.2 | Replay (과거 실행 흐름 재현) | ✅ `ReplayService`, `ReplayTests` |
| §3.5 | 정책·권한 엔진 (속성/시간/이력 조건, RBAC 금지, 무재시작 반영) | ✅ `PolicyEngine`, `PolicyTests` |

`mvn test` → **69/69 pass**.

### 설계상 보류 (스펙 범위 밖으로 명시 결정)

- 비동기 실행 전용 스케줄러/큐 기반 디스패치 (현재는 스레드풀 직접 실행)
- 외부 분산 락 backend 통합 (Redis/etcd) — 추상화는 완료, 실제 어댑터는 미포함

### 주요 API

- `POST /workflows`, `GET /workflows/{name}/versions`
- `POST /executions` (`{workflow, context}`), `POST /executions/async`, `GET /executions/{id}`
- `POST /executions/{id}/approve|reject`
- `GET /executions/{id}/events|audit|replay`
- `POST /admin/executions/{id}/cancel`, `/steps/{stepId}/rerun`, `/force-status`; `GET /admin/executions/dead`, `/admin/stats`
- `POST /policies`, `GET /policies`, `PATCH /policies/{id}/enabled`, `DELETE /policies/{id}`
